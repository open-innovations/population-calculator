...
/*
		...
		var f = FetchExtract();
		f.get("areas/area_meta.db",function(txt,attr){
			rows = txt.split(/\n/);
			for(var r = 0; r < rows.length; r++){
				cols = rows[r].split(/\t/);
				rows[r] = {'a':cols[0],'b':cols[1],'start':parseInt(cols[2]),'end':parseInt(cols[3])};
			}
			console.log(rows[1]);
			f.get("areas/area_full.db",function(txt,attr){
				console.log(txt,attr);
			},{'start':rows[1].start,'end':rows[1].end});
		});
		//f.get("https://open-innovations.github.io/parkulator/",function(txt,attr){ console.log('rtn',txt,txt.length,attr); },{'start':1,'end':50});
...
*/
		...
		// A function to convert into plain ascii (as that is what we have in the search results)
