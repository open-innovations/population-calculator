/*
	This is a modified version of https://github.com/bopen/leaflet-area-selection version 0.7.0 (May 2023)
...
(function (global, factory) {
	...
	var styles = {"leafletAreaSelectorControl":"_37qQG","leafletMapOverlayPane":"leaflet-area-selection","drawPaneHelp":"","endSelectionArea":"","areaSelectMarker":"leaflet-area-selector","areaSelectGhostMarker":"leaflet-area-selector"};
	...
	function onGhostMarkerDrag(index) {
		...
		return function (event) {
			...
			requestAnimationFrame(function () {
				var firstPoint = _this5.markers[index];
				var lastPoint = _this5.markers[index + 1] ? _this5.markers[index + 1] : _this5.markers[0];

				if (_this5.ghostPolygon) {
					map.removeLayer(_this5.ghostPolygon);
				}

				_this5.ghostPolygon = new leaflet.Polygon([firstPoint.marker.getLatLng(), latlng, lastPoint.marker.getLatLng()], {
				...
				map.addLayer(_this5.ghostPolygon);
				map.fire('as:update-polygon');
			});
		};
	}
	...
	var DrawAreaSelection = leaflet.Control.extend({
		...
		onAdd: function onAdd(map) {
			this._container = leaflet.DomUtil.create('div', cls('leaflet-area-selector-control'));
		...
		startPolygon: function(){
			...
			var activeState = this.options.active || this.phase === 'adjust';
			if(activeState) this.deactivate();
